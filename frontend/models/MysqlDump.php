...
class MysqlDump {
	...
	function dbDump($host, $user, $pwd, $db, $table = null, $filename = null) {
		$mysqlconlink = mysql_connect($host, $user, $pwd, true);
		$this->file_path = null;
		if (!$mysqlconlink) {
			$this->error_msg = sprintf('No MySQL connection: %s', mysql_error());
			return false;
		}
		...
		if (!$mysqldblink) {
			$this->error_msg = sprintf('No MySQL connection to database: %s', mysql_error());
			return false;
		}
		...
		if (!$result) {
			$this->error_msg = sprintf('Database error %1$s for query %2$s', mysql_error(), "SHOW TABLE STATUS FROM `$db`;");
			return false;
		}
		...
		if (count($tabelstobackup) > 0) {
			...
			if (!isset($filename)) {
				$date = date('YmdHis');
				$filename = "files/db/{$db}.{$date}.sql";
			}
			if ($file = fopen($filename, 'wb')) {
				fwrite($file, "-- ---------------------------------------------------------\n");
				fwrite($file, "-- Database Name: $db\n");
				if (empty($table)) {
					//if not only dump single table, dump database create sql
				...
				$this->error_msg = 'Can not create database dump!';
				return false;
			}
		} else {
			$this->error_msg = 'No tables to dump';
			return false;
		}
		return false;
	}
	...
	protected static function _db_dump_create_database($dbname, $file) {
		...
		if (!$result) {
			echo sprintf('Database error %1$s for query %2$s', mysql_error(), $sql);
			return false;
		}
	...
	protected static function need_free_memory($memneed) {
		...
		if ($needmemory > self::inbytes(ini_get('memory_limit'))) {
			...
			if ($oldmem = @ini_set('memory_limit', $newmemory)) {
				echo sprintf('Memory increased from %1$s to %2$s', 'backwpup', $oldmem, @ini_get('memory_limit'));
			} else {
				echo sprintf('Can not increase memory limit is %1$s', 'backwpup', @ini_get('memory_limit'));
			}
